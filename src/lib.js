...
// Audit-log before→after detail, formatted by value kind. `kind` is carried
// through so the audit view can show short money/% diffs inline and collapse
// long text diffs behind an "inspect" toggle.
export function valDetail(oldV, newV, kind) {
  const k = kind || "text";
  const f = k === "pct" ? pct : k === "money" ? money : (x) => (x == null || x === "" ? "—" : String(x));
  return { before: f(oldV), after: f(newV), kind: k };
}
